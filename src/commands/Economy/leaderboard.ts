...
export default class extends Command {
    ...
    async execute(interaction: ICommandInteraction) {
        const profiles = await sneekyProfile.find({
        ...
        if (!profiles.length)
            return interaction.reply({
                embeds: [
                    new MessageEmbed()
                        ...
                        .setDescription(
                            "Unable to find any guild profiles to populate the leaderboard"
                        )
                        .setTimestamp()
                        .setFooter({
            ...
            const sorted = profiles.sort(
                (a, b) => b.bank + b.wallet - a.bank + a.wallet
            );
            ...
            top10.forEach((profile: any) => {
                ...
                embed.addFields({
                    ...
                    value: `Wallet: ${"$" + profile.wallet} Bank: ${
                        "$" + profile.bank
                    }`,
                    inline: false,
                });
