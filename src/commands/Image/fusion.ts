...
export default class extends Command {
        ...
        super(bot, {
            ...
            options: [
                ...
                },
                {
                    name: "overlay",
                    description:
                        "User for the overlay profile (empty chooses you)",
                    required: false,
                    type: "USER",
                },
                {
    ...
    async execute(interaction: ICommandInteraction) {
        ...
        const overlay =
            (await interaction.options.getUser("overlay")) || interaction.user;
        ...
        const baseURL =
            ...
            interaction.options.get("type")?.value === "server_avatar"
                ? base?.displayAvatarURL({ format: "jpg", size: 512 })
                : base?.avatarURL({ format: "jpg", size: 512 });

        const overlayURL =
            !interaction.options.get("type") ||
            interaction.options.get("type")?.value === "server_avatar"
                ? overlay.displayAvatarURL({ format: "jpg", size: 512 })
                : overlay.avatarURL({ format: "jpg", size: 512 });
        ...
        const canvas = createCanvas(
            baseAvatarImage.width,
            baseAvatarImage.height
        );
