...
export default class extends Command {
    ...
    async execute(interaction: ICommandInteraction) {
        ...
        if (data.definition.length > 1000)
            definition =
                data.definition.slice(0, 950) +
                `... [Read More](${data.permalink})`;
        else if (data.example > 1000)
            example =
                data.example.slice(0, 950) +
                `... [Read More](${data.permalink})`;
        else if (data.author > 1000)
            author =
                data.author.slice(0, 950) +
                `... [Read More](${data.permalink})`;
            ...
            if (!interaction.channel.nsfw) {
                return interaction.reply({
                    embeds: [
                        new MessageEmbed()
                            .setTitle(
                                `Definition for: ${wordFilter.clean(
                                    data.word.replace(
                                        /([a-zA-Z])([A-Z])([a-z])([a-zA-Z])([A-Z])([a-z])/g,
                                        data.word
                                    )
                                )}`
                            )
